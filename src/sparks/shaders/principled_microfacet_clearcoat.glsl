...
Spectrum bsdf_microfacet_ggx_eval_reflect_clearcoat(const MicrofacetBsdf bsdf,
                                                    const float3 N,
                                                    const float3 I,
...
Spectrum bsdf_microfacet_ggx_eval_transmit_clearcoat(const MicrofacetBsdf bsdf,
                                                     const float3 N,
                                                     const float3 I,
                                                     const float3 omega_in,
                                                     inout float pdf,
...
Spectrum bsdf_microfacet_ggx_eval_clearcoat(const MicrofacetBsdf bsdf,
                                            const float3 I,
                                            const float3 omega_in,
                                            inout float pdf) {
  ...
  return (cosNI < 0.0f)
             ? bsdf_microfacet_ggx_eval_transmit_clearcoat(
                   bsdf, N, I, omega_in, pdf, alpha_x, alpha_y, cosNO, cosNI)
             : bsdf_microfacet_ggx_eval_reflect_clearcoat(
                   bsdf, N, I, omega_in, pdf, alpha_x, alpha_y, cosNO, cosNI);
}

int bsdf_microfacet_ggx_sample_clearcoat(const MicrofacetBsdf bsdf,
                                         float3 Ng,
                                         float3 I,
                                         float randu,
                                         float randv,
